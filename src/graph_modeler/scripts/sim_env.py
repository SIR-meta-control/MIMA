...
from commons.base import MujocoEnv
...
class CrimsonMujocoEnv(MujocoEnv):
    ...
    def __init__(self, **kwargs):
        model_path = Path(f"./models/crimson/mjcf/crimson_scene.xml")
        MujocoEnv.__init__(
            self,
    ...
    def get_joint_torque(self):
        """获取指定关节对应执行器所施加的扭矩"""
        # 关节名称列表
        target_joints = ["frameJoint2", "frameJoint3", "frameJoint5", "frameJoint6", "frameJoint8",
                         "leg0Joint1", "leg0Joint2", "leg0Joint3", "leg1Joint1", "leg1Joint2", "leg1Joint3",
                         "leg2Joint1", "leg2Joint2", "leg2Joint3", "leg3Joint1", "leg3Joint2", "leg3Joint3"]
        # 存储扭矩的列表
        joint_torques = []
        ...
        for joint_name in target_joints:
            ...
            for act_id in range(self.model.nu):  # self.model.nu 是执行器的总数
                ...
                is_joint_actuator = self.model.actuator_trntype[act_id] == mujoco.mjtTrn.mjTRN_JOINT
