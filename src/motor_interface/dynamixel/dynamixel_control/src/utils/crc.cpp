...
unsigned short update_crc(unsigned short crc_accum,
                          std::vector<unsigned char> &data_blk_ptr,
...
void CalcCRC(unsigned char *CRC_L, unsigned char *CRC_H,
             std::vector<unsigned char> &data, int dataLength) {
  unsigned short crc = update_crc(0, data, dataLength + 5);
  *CRC_L = crc & 0xFF;
  *CRC_H = (crc >> 8) & 0xFF;
}

bool CheckCRC(std::string buffer) {
  std::vector<unsigned char> list = {0xff, 0xff, 0xfd};
  for (size_t i = 0; i < buffer.length() - 2; i++)
    list.emplace_back(static_cast<unsigned char>(buffer[i]));
  unsigned char crcl, crcu;
  CalcCRC(&crcl, &crcu, list, buffer[2]);
  bool res = (crcl == buffer[buffer.length() - 2]) &&
             (crcu == buffer[buffer.length() - 1]);
