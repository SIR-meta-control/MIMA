...
void GroupSyncRead::makeParam() {
  if (ph_->getProtocolVersion() == 1.0 || id_list_.size() == 0) return;

  if (param_ != 0) delete[] param_;
  param_ = 0;
...
bool GroupSyncRead::addParam(uint8_t id) {
  if (ph_->getProtocolVersion() == 1.0) return false;

  if (std::find(id_list_.begin(), id_list_.end(), id) !=
      id_list_.end())  // id already exist
    return false;
...
void GroupSyncRead::removeParam(uint8_t id) {
  if (ph_->getProtocolVersion() == 1.0) return;

  std::vector<uint8_t>::iterator it =
      std::find(id_list_.begin(), id_list_.end(), id);
...
int GroupSyncRead::txPacket() {
  ...
  if (is_param_changed_ == true || param_ == 0) makeParam();

  return ph_->syncReadTx(port_, start_address_, data_length_, param_,
                         (uint16_t)id_list_.size() * 1);
}

int GroupSyncRead::rxPacket() {
  last_result_ = false;

  if (ph_->getProtocolVersion() == 1.0) return COMM_NOT_AVAILABLE;

  int cnt = id_list_.size();
  int result = COMM_RX_FAIL;

  if (cnt == 0) return COMM_NOT_AVAILABLE;

  for (int i = 0; i < cnt; i++) {
    uint8_t id = id_list_[i];

    result =
        ph_->readRx(port_, id, data_length_, data_list_[id], error_list_[id]);
    if (result != COMM_SUCCESS) return result;
  }

  if (result == COMM_SUCCESS) last_result_ = true;
...
bool GroupSyncRead::isAvailable(uint8_t id, uint16_t address,
                                uint16_t data_length) {
  if (ph_->getProtocolVersion() == 1.0 || last_result_ == false ||
      data_list_.find(id) == data_list_.end())
    return false;
...
uint32_t GroupSyncRead::getData(uint8_t id, uint16_t address,
                                uint16_t data_length) {
  ...
  switch (data_length) {
    ...
    case 2:
      return DXL_MAKEWORD(data_list_[id][address - start_address_],
                          data_list_[id][address - start_address_ + 1]);

    case 4:
      return DXL_MAKEDWORD(
          DXL_MAKEWORD(data_list_[id][address - start_address_ + 0],
                       data_list_[id][address - start_address_ + 1]),
...
bool GroupSyncRead::getError(uint8_t id, uint8_t *error) {
  ...
  // if (ph_->getProtocolVersion() == 1.0 || last_result_ == false ||
  // error_list_.find(id) == error_list_.end())
