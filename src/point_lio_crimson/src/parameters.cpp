...
void readParameters(ros::NodeHandle &nh) {
  ...
  nh.param<double>("common/cut_frame_time_interval", cut_frame_time_interval,
                   0.1);
  ...
  nh.param<bool>("mapping/start_in_aggressive_motion", non_station_start,
                 false);
  ...
  nh.param<std::vector<double>>("mapping/gravity", gravity,
                                std::vector<double>());
  nh.param<std::vector<double>>("mapping/gravity_init", gravity_init,
                                std::vector<double>());
  nh.param<std::vector<double>>("mapping/extrinsic_T", extrinT,
                                std::vector<double>());
  nh.param<std::vector<double>>("mapping/extrinsic_R", extrinR,
                                std::vector<double>());
  nh.param<bool>("odometry/publish_odometry_without_downsample",
  ...
  nh.param<std::string>("map_show_path", map_show_path_, "");
  nh.param<double>("initial_z", initial_z_, 0.0);
  nh.param<int>("robot_config", robot_config_, 3);
}
