...
uint64_t lose_lidar_cnt = 0;

int feats_down_size = 0, time_log_counter = 0, scan_count = 0,
    publish_count = 0;
...
void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  if (cut_frame) {
    ...
    for (int i = 0; i < ptr->size(); i++) {
      ...
      if (ptr->points[i].curvature / double(1000) + msg->header.stamp.toSec() -
              time_div >
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in) {
  ...
  msg->header.stamp =
      ros::Time().fromSec(msg_in->header.stamp.toSec() - time_lag_imu_to_lidar);
...
bool sync_packages(MeasureGroup &meas) {
  if (!imu_en) {
    if (!lidar_buffer.empty()) {
      ...
      if (meas.lidar->points.size() < 1) {
        cout << "lose lidar " << lose_lidar_cnt << std::endl;
        lose_lidar_cnt++;
        return false;
      }
  ...
  if (!lidar_pushed) {
    meas.lidar = lidar_buffer.front();
    if (meas.lidar->points.size() < 1) {
      cout << "lose lidar " << lose_lidar_cnt << endl;
      lose_lidar_cnt++;
      lidar_buffer.pop_front();
      time_buffer.pop_front();
      return false;
  ...
  for (int i = 0; i < feats_down_size; i++) {
    if (!Nearest_Points[i].empty()) {
      ...
      for (int readd_i = 0; readd_i < points_near.size(); readd_i++) {
        /* Those points which are outside the downsample box should not be
         * considered. */
        if (fabs(points_near[readd_i].x - mid_point.x) <
                0.5 * filter_size_map_min &&
            fabs(points_near[readd_i].y - mid_point.y) <
                0.5 * filter_size_map_min &&
            fabs(points_near[readd_i].z - mid_point.z) <
                0.5 * filter_size_map_min) {
          need_add = false;
          break;
        }
...
void init_pose_cbk(
  ...
  init_rotation_ = Eigen::Quaterniond(
  ...
  Eigen::Quaterniond rotation_pai_2(
      Eigen::AngleAxisd(M_PI / 2, Eigen::Vector3d::UnitX()));

  Eigen::Quaterniond rotation_pai(
      Eigen::AngleAxisd(-M_PI, Eigen::Vector3d::UnitX()));
  if (robot_config_ < 3) {
    init_rotation_ = init_rotation_ * rotation_pai_2;
  } else if (robot_config_ < 5) {
    init_rotation_ = init_rotation_ * rotation_pai;
  }
...
void initial_pose() {
  ...
  if (icp.hasConverged() == false || score == 0.0 || score > 2) {
    ...
    ROS_INFO("Global Initializing Success with %f!", score);
    ...
    ROS_INFO("\033[1;35m Initializing Succeed with %f score! \033[0m", score);
    ROS_INFO("\033[1;35m Initializing Position: %f %f %f,%f %f %f %f.\033[0m",
...
int main(int argc, char **argv) {
  ...
  kf_output.init_dyn_share_modified_2h(get_f_output, df_dx_output,
  ...
  if (p_pre->lidar_type == AVIA) {
    ...
    sub_pcl = nh.subscribe(lid_topic, 200000, livox_pcl_cbk2);
  } else {
  ...
  if (flg_islocation_mode_) {
    ...
    pcl::io::loadPCDFile(map_show_path_, *map_show_cloud);
    ROS_INFO("Load map cloud with size: %zu.", map_show_cloud->points.size());
    ...
    if (sync_packages(Measures)) {
        ...
        if (!p_imu->gravity_align_) {
          ...
          if (non_station_start) {
            ...
            if (!flg_islocation_mode_) {
              ...
              Eigen::Vector3d new_gravity = p_imu->mean_acc * G_m_s2 / acc_norm;
              state_out.acc = new_gravity;
              int max_abs_index;
              Eigen::AngleAxisd rotation(0.0, Eigen::Vector3d::UnitX());
              new_gravity.cwiseAbs().maxCoeff(&max_abs_index);
              if (new_gravity(max_abs_index) > 0) {
                new_gravity *= -1;
              }
              if (max_abs_index == 0) {
                if (new_gravity(max_abs_index) > 0) {
              ...
              new_gravity = rotation * new_gravity;
              std::cout << new_gravity.transpose() << std::endl;
              state_in.gravity = new_gravity;
              state_out.gravity = new_gravity;
            }
          }
          if (gravity_align) {
          ...
          std::cout << "gravity: " << state_in.gravity.transpose() << std::endl;
          std::cout << "-------------------" << std::endl;
      ...
      if (!use_imu_as_input) {
        ...
        for (k = 0; k < time_seq.size(); k++) {
          ...
          if (imu_en) {
            ...
            while (imu_comes) {
              ...
              imu_deque.pop_front();
              double dt =
                  imu_last.header.stamp.toSec() - time_predict_last_const;
              ...
              {
                double dt_cov =
                    imu_last.header.stamp.toSec() - time_update_last;
          ...
          // {
          // double dt_cov = time_current - time_update_last;
          // if ((dt_cov >= imu_time_inte)) // (point_cov_not_prop &&
          // imu_prop_cov)
          // {
        ...
        for (k = 0; k < time_seq.size(); k++) {
          ...
          while (time_current >
                 imu_next.header.stamp.toSec())  // && !imu_deque.empty())
          {
            imu_last = imu_next;
            imu_next = *(imu_deque.front());
            imu_deque.pop_front();
            ...
            if (dt_cov > 0.0) {
              ...
              time_update_last =
                  imu_last.header.stamp.toSec();  // time_current;
            }
          ...
          // {
          //     double dt_cov = time_current - time_update_last;
          //     if ((imu_prop_cov && dt_cov > 0.0) || (dt_cov >=
          //     imu_time_inte
          //     * 1.2))
          //     {
          //         double propag_cov_start = omp_get_wtime();
