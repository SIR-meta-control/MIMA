...
from .configuration_intern_pcmae import InternPointCloudConfig
...
def fps(data, number):
    ...
    number int
    """
    fps_idx = pointnet2_utils.furthest_point_sample(data.float(), number)
    fps_data = (
        pointnet2_utils.gather_operation(
            data.float().transpose(1, 2).contiguous(), fps_idx
        )
        .transpose(1, 2)
        .contiguous()
    )
    return fps_data.to(torch.bfloat16), fps_idx.to(torch.bfloat16)
...
class InternPointCloudModel(PreTrainedModel):
    main_input_name = "point_cloud"
    config_class = InternPointCloudConfig
    _no_split_modules = ["TransformerEncoder"]

    def __init__(self, config: InternPointCloudConfig):
        ...
        self.use_point_cloud_model = config.use_point_cloud_model
    ...
    def load_model_from_ckpt(self, mae_ckpt_path):
        if mae_ckpt_path is not None:
            ckpt = torch.load(mae_ckpt_path)
            base_ckpt = {
            ...
            print(f"[Transformer] Successful Loading the ckpt from {mae_ckpt_path}")
    ...
    def forward(
        ...
        if self.encoder_dims != self.trans_dim:
            ...
            B, R, N, C = point_cloud.shape
            point_cloud = point_cloud.reshape(B * R, N, C)
            neighborhood, center, ori_idx, center_idx = self.group_divider(point_cloud)
            # # generate mask
...
# AutoModel.register(InternPointCloudConfig, InternPointCloudModel)
