...
from internvl.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      ...
                                      REF_START_TOKEN, PC_START_TOKEN, PC_END_TOKEN, PC_CONTEXT_TOKEN)
...
class ModelArguments:
    ...
    freeze_pc_backbone: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the vision backbone of the model.'},
    )
    freeze_mlp1: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP1 layers of the model.'},
    )
    freeze_mlp2: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP2 layers of the model.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, template_name, meta, tokenizer, tcs_loader, num_image_token,
        ...
        self.num_point_cloud_token = num_image_token
        self.npoints = 2048
                ...
                if 'length' in data_item:
                    ...
                    if str_length not in self.conv2length:
                        ...
                        self.conv2length[str_length] = token_length + num_image_token * (
                                    max_dynamic_patch + use_thumbnail) + self.num_point_cloud_token * (max_dynamic_patch + use_thumbnail)
                    else:
    ...
    def load_point_cloud(self, file_path):
        ...
        pcd = self.random_sample(pcd, self.sample_points_num)
        pcd = self.pc_norm(pcd)
        pcd = torch.from_numpy(pcd).float()
        # pdb.set_trace()
        return pcd

    def multi_modal_pc_img_get_item(self, data_item):
        # pdb.set_trace()
        if '<pointcloud>' not in data_item['conversations'][0]['value']:
        ...
        # pdb.set_trace()
        pointcloud_path = os.path.join(self.root, data_item['pointcloud'])
        point_cloud = self.load_point_cloud(pointcloud_path)
        point_cloud = point_cloud.unsqueeze(0)
        point_cloud = point_cloud.expand(num_patches, -1, -1)
        # pdb.set_trace()
        ...
        # print('data[conversations is]', data_item['conversations'])
        # print('len of data[conversations is]', data_item['conversations'].shape)
        ...
        ret = dict(
            ...
            point_cloud_flags=torch.tensor([1] * num_patches, dtype=torch.long),
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            try:
                ...
                # pdb.set_trace()
                if 'image' in data_item and 'pointcloud' in data_item:
                    ret = self.multi_modal_pc_img_get_item(data_item)
...
def main():
    ...
    #launcher = os.environ.get('LAUNCHER', 'slurm', 'pytorch')
    ...
    token_list = [IMG_START_TOKEN, IMG_END_TOKEN, IMG_CONTEXT_TOKEN,
                  ...
                  REF_END_TOKEN, BOX_START_TOKEN, BOX_END_TOKEN, PC_START_TOKEN, PC_END_TOKEN, PC_CONTEXT_TOKEN]
    ...
    model.pc_context_token_id = pc_context_token_id
    ...
    # print('number of model.num_image_token is:',model.num_image_token)
    ...
    if model_args.freeze_pc_backbone:
        # model.vision_model = model.vision_model.eval()
        _freeze_params(model.point_cloud_model)
