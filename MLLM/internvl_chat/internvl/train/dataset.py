...
from .constants import (CLIP_MEAN, CLIP_STD, IMAGENET_MEAN, IMAGENET_STD,
                        ...
                        SIGLIP_MEAN, SIGLIP_STD, PC_CONTEXT_TOKEN, PC_START_TOKEN, PC_END_TOKEN)
...
def preprocess_phi3(
    ...
    for i, source in enumerate(sources):
        if roles[source[0]['from']] != conv.roles[0]:
            ...
            # print('len of source is', source.shape)
        conv.messages = []
        for j, sentence in enumerate(source):
            role = roles[sentence['from']]
            assert role == conv.roles[j % 2], f'{i}'
    ...
    # pdb.set_trace()
    image_tokens = f'{IMG_START_TOKEN}{IMG_CONTEXT_TOKEN * num_image_token}{IMG_END_TOKEN}'
    pointcloud_tokens = f'{PC_START_TOKEN}{PC_CONTEXT_TOKEN * num_image_token}{PC_END_TOKEN}'
    # pdb.set_trace()
    if not text_only:
        new_conversations = []
        for conversation in conversations:
            conversation = conversation.replace('<image>', image_tokens, num_image)
            conversation = conversation.replace('<pointcloud>', pointcloud_tokens, num_pointcloud)
            new_conversations.append(conversation)
        conversations = new_conversations
    # pdb.set_trace()
    # Tokenize conversations
