...
mkdir -p logs
log_path="logs/mlp_gnn_ablation_$(date +%Y%m%d_%H%M%S).log"
...
python -u scripts/train/train_mlp_ablation.py \
  ...
  --output-dir "${CKPT_DIR}/mlp_gnn_ablation" \
  --hidden-dims 272 272 272 \
  --scale-mode gnn \
  --bar-embedding-dim 16 \
  --scale-gnn-hidden-dims 128 256 256 \
  --epochs 40 \
  --batch-size 512 \
  --lr 0.001 \
  --weight-decay 0.0001 \
